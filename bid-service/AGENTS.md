...
- Feign 오류 매핑 두 겹: HTTP 응답이 있는 오류는 `AuctionClientErrorDecoder`(404→404, 5xx→503, 그 외 4xx→502), 연결 실패·타임아웃은 `BidService.fetchAuction`의 `FeignException` catch(→503). 한 겹만 있으면 상류가 꺼졌을 때 500이 나간다.
...
- 상류 404/연결 실패/5xx/필드 누락 → 404/503/503/502
- 낙찰 확정 멱등성, 입찰 없음 응답
- 동시 입찰 직렬화 (D7)
