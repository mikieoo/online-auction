...
- Gateway는 경로를 정규화하지 않고 원문 그대로 하류에 넘긴다. 그래서 인증 필터가 raw 경로에 `%`, `;`, `\`, `..`, `//`가 있으면 토큰과 무관하게 404로 끊는다(`/api/v1/bids/../../internal/...`류 우회와 `/api/v1/payments;x=1/1`류 인증 판정 우회 차단). 이 API의 경로는 영문·숫자·`/`·`-`만 쓴다 — 경로에 인코딩이 필요한 값을 넣는 API를 추가하면 이 규칙과 충돌한다.
...
- 라우트·브레이커·TimeLimiter·Eureka·actuator 노출은 전부 `application.yml`. 브레이커 튜닝 값은 `resilience4j.circuitbreaker.configs.default` 아래에 둔다 — Spring Cloud CircuitBreaker 팩토리는 `configs.{이름}` → `configs.default` 순으로 설정을 찾는다. 서비스별로 다르게 하려면 `configs.bid-service`처럼 추가한다.
...
- 경로 우회 변형(`..`, `%2e%2e`, `..;`, `;x=1`, `%70`, `//`) → 404, 체인 미호출
